...
class KafkaStreamsConfig(
    ...
    fun psbKafkaStreamsBuilder(): StreamsBuilderFactoryBean {
        val psbStreamProps = kafkaProperties.streams["psb"] ?: throw IllegalStateException("Mangler konfiguration for psb streams")
    ...
    private fun commonStreamsConfigProperties(
        ...
        props["retries"] = kafkaProperties.retries
        props[StreamsConfig.RETRY_BACKOFF_MS_CONFIG] = 1000
    ...
    private fun StreamsBuilderFactoryBean.configure() {
        ...
        addListener(object : StreamsBuilderFactoryBean.Listener {
            override fun streamsAdded(id: String, streams: KafkaStreams) {
                logger.info("KafkaStream added: $id")
                super.streamsAdded(id, streams)
            }

            override fun streamsRemoved(id: String, streams: KafkaStreams) {
                logger.info("KafkaStream removed: $id")
                super.streamsRemoved(id, streams)
            }
        })
