...
class PSBSøknadPdfData(
    private val søknad: PSBMottattSøknad,
) : PdfData() {
    ...
    private fun nattevåk(nattevaak: Nattevåk?) =
        when {
            nattevaak == null -> null
            else -> {
                mapOf(
    ...
    private fun beredskap(beredskap: Beredskap?) =
        when {
            beredskap == null -> null
            else -> {
                mapOf(
    ...
    private fun PlanUkedager.somMap(avkort: Boolean = true) =
        mapOf<String, Any?>(
    ...
    private fun ArbeidsRedusert.somMap() =
        mapOf(
            "type" to this.type.name,
    ...
    private fun List<ArbeidsUke>.somMap() =
        sortedBy { it: ArbeidsUke -> it.periode.fraOgMed }
            .map {
    ...
    private fun NormalArbeidstid.somMap(): Map<String, Any?> =
        mapOf(
            "timerPerUkeISnitt" to this.timerPerUkeISnitt.tilString(),
        )

    private fun Frilans.somMap(søknadsperiodeStartdato: LocalDate): Map<String, Any?> =
        mapOf(
    ...
    private fun SelvstendigNæringsdrivende.somMap(): Map<String, Any?> =
        mapOf(
    ...
    private fun YrkesaktivSisteTreFerdigliknedeÅrene.somMap(): Map<String, Any?> =
        mapOf(
            "oppstartsdato" to Constants.DATE_FORMATTER.format(oppstartsdato),
        )

    private fun VarigEndring.somMap(): Map<String, Any?> =
        mapOf(
    ...
    private fun StønadGodtgjørelse.somMap() =
        mapOf(
