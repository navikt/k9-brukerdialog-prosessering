...
@Component
class AktivitetspengersøknadJournalføring(
    ...
    @param:Qualifier(AktivitetspengersøknadTopologyConfiguration.AKTIVITETSPENGER_SØKNAD_JOURNALFØRING_STREAMS_BUILDER_NAME) private val streamsBuilder: StreamsBuilder,
    @param:Qualifier(AktivitetspengersøknadTopologyConfiguration.AKTIVITETSPENGER_SØKNAD_JOURNALFØRING_STREAMS_BUILDER_NAME) private val streamsBuilderFactoryBean: StreamsBuilderFactoryBean,
) : HealthIndicator {

    private companion object {
        private val STREAM_NAME = KafkaStreamName.AKTIVITETSPENGER_SØKNAD_JOURNALFØRING
        private val logger = LoggerFactory.getLogger(AktivitetspengersøknadJournalføring::class.java)
    }

    @Bean
    fun aktivitetspengersøknadJournalføringsStream(): KStream<String, TopicEntry<AktivitetspengersøknadPreprosessertSøknad>> {
        val stream = streamsBuilder
            .stream(aktivitetspengersøknadPreprosessertTopic.name, aktivitetspengersøknadPreprosessertTopic.consumedWith)

        stream
            .process(ProcessorSupplier { LoggingToMDCProcessor() })
            .mapValues { _: String, value: TopicEntry<AktivitetspengersøknadPreprosessertSøknad> ->
            ...
            .to(aktivitetspengersøknadCleanupTopic.name, aktivitetspengersøknadCleanupTopic.producedWith)
