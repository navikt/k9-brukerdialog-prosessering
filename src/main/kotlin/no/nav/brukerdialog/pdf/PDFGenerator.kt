...
class PDFGenerator {
    ...
    private companion object {
        ...
        private fun configureHandlebars() =
            Handlebars(ClassPathTemplateLoader("/$ROOT")).apply {
                imageHelper()
                equalsHelper()
                equalsNumberHelper()
                equalsJaNeiHelper()
    ...
    fun genererPDF(pdfData: PdfData): ByteArray =
        genererHTML(pdfData).let { html ->
    ...
    fun genererHTML(pdfData: PdfData): String =
        template(pdfData)
            .apply(
                Context
...
abstract class PdfData {
    ...
    open fun nyPdfData(): List<VerdilisteElement> = emptyList()
