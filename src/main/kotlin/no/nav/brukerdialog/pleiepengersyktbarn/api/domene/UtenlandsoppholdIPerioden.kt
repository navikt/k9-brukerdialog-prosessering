...
data class Utenlandsopphold(
    ...
    val perioderBarnetErInnlagt: List<no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Periode> = listOf(),
    val årsak: no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Årsak?,
) {
    ...
    fun somUtenlandsoppholdPeriodeInfo() = UtenlandsoppholdPeriodeInfo()
        ...
        .apply {
            if (årsak != null && årsak != no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Årsak.ANNET) {
...
data class UtenlandsoppholdIPerioden(
    ...
    val opphold: List<no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Utenlandsopphold> = listOf(),
) {
    internal fun tilK9Utenlandsopphold(): K9Utenlandsopphold {
        ...
        opphold.forEach { utenlandsopphold ->
            ...
            val gjenværendePerioderUtenInnleggelse = tidslinjeUtenInnleggelse.toSegments().map {
                no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Periode(it.fom, it.tom)
            }
...
enum class Årsak {
    ...
    fun tilK9Årsak() = when (this) {
        no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Årsak.BARNET_INNLAGT_I_HELSEINSTITUSJON_FOR_NORSK_OFFENTLIG_REGNING -> K9UtenlandsoppholdÅrsak.BARNET_INNLAGT_I_HELSEINSTITUSJON_FOR_NORSK_OFFENTLIG_REGNING
        no.nav.brukerdialog.pleiepengersyktbarn.api.domene.Årsak.BARNET_INNLAGT_I_HELSEINSTITUSJON_DEKKET_ETTER_AVTALE_MED_ET_ANNET_LAND_OM_TRYGD -> K9UtenlandsoppholdÅrsak.BARNET_INNLAGT_I_HELSEINSTITUSJON_DEKKET_ETTER_AVTALE_MED_ET_ANNET_LAND_OM_TRYGD
        else -> null
    }
}
