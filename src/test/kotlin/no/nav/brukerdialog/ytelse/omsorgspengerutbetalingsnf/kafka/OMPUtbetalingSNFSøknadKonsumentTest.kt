package no.nav.brukerdialog.ytelse.omsorgspengerutbetalingsnf.kafka
...
import no.nav.brukerdialog.ytelse.omsorgpengerutbetalingsnf.kafka.OMPUtbetalingSNFTopologyConfiguration
...
import no.nav.brukerdialog.ytelse.omsorgspengerutbetalingsnf.utils.OMPUtbetalingSNFSøknadUtils
...
class OMPUtbetalingSNFSøknadKonsumentTest : AbstractIntegrationTest() {
    ...
    override val consumerGroupTopics = listOf(
        OMPUtbetalingSNFTopologyConfiguration.OMP_UTB_SNF_MOTTATT_TOPIC,
        OMPUtbetalingSNFTopologyConfiguration.OMP_UTB_SNF_PREPROSESSERT_TOPIC,
        OMPUtbetalingSNFTopologyConfiguration.OMP_UTB_SNF_CLEANUP_TOPIC
    )
    ...
    fun `forvent at melding konsumeres riktig og dokumenter blir slettet`() {
        ...
        k9DittnavVarselConsumer.lesMelding(
            ...
            K9Beskjed(
                ...
                eventId = "testes ikke",
                ytelse = "OMSORGSPENGER_UTBETALING_SNF",
            )
        )
    }
    ...
    fun `Forvent at melding bli prosessert på 5 forsøk etter 4 feil`() {
        ...
        producer.leggPåTopic(key = søknadId, value = topicEntryJson, topic = OMPUtbetalingSNFTopologyConfiguration.OMP_UTB_SNF_MOTTATT_TOPIC)
        val lesMelding =
            consumer.lesMelding(key = søknadId, topic = OMPUtbetalingSNFTopologyConfiguration.OMP_UTB_SNF_PREPROSESSERT_TOPIC).value()
